...
#------------------------ Single vaccionation coverage 
df_obitos_v1 <- df_obitos


#------------------------ Full vaccionation coverage
df_obitos_v2 <- df_obitos



# df_obitos$cobertura_single <- df_obitos_v1$cobertura
# df_obitos$cobertura_full <- df_obitos_v2$cobertura
# View(df_obitos)
...
filtro_20_49 <- df_obitos %>% 
  ...
  mutate(cobertura_single = round(cobertura_single,4))
c75.1 <- filtro_20_49$data[which(filtro_20_49$cobertura_single > 0.75)][1]

filtro_50_59 <- df_obitos %>% 
  filter(faixa_etaria == "50-59") %>% 
  mutate(cobertura_single = round(cobertura_single,4))
c75.2 <- filtro_50_59$data[which(filtro_50_59$cobertura_single > 0.75)][1]

filtro_60_69 <- df_obitos %>% 
  filter(faixa_etaria == "60-69") %>% 
  mutate(cobertura_single = round(cobertura_single,4))
c75.3 <- filtro_60_69$data[which(filtro_60_69$cobertura_single > 0.75)][1]

filtro_70 <- df_obitos %>% 
  filter(faixa_etaria == "70+") %>% 
  mutate(cobertura_single = round(cobertura_single,4))
c75.4 <- filtro_70$data[which(filtro_70$cobertura_single > 0.75)][1]

#----------------------- Figure single dose
c1 <- df_obitos %>% 
  filter(faixa_etaria %in% c("20-49", "50-59", "60-69", "70+"), data > "2020-12-31") %>% 
  mutate(cobertura_single = if_else(faixa_etaria == "20-49" & cobertura_single > 1, 1, cobertura_single)) %>% 
  ggplot() +
  geom_line(aes(x = data, y = cobertura_single, color = faixa_etaria), size = 1) +
...
ggsave("vaccine_coverage_single_full_v1.png", width = 7, height = 4)
